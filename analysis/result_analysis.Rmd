...
cat("==== Overall Summary ====\n")
overall_summary <- result_small %>%
  summarise(
    overall_mean_ari = round(mean(ari, na.rm = TRUE), 4),
    overall_sd_ari = round(sd(ari, na.rm = TRUE), 4),
    overall_mean_tucker_S = round(mean(tucker_S, na.rm = TRUE), 4),
    overall_sd_tucker_S = round(sd(tucker_S, na.rm = TRUE), 4)
  )
...
anova_table <- summary(anova_inter)[[1]]

# Total SS is the sum of all Sum Sq (including residuals)
ss_total <- sum(anova_table[["Sum Sq"]])

# Calculate eta squared for each factor
eta_squared <- anova_table[["Sum Sq"]] / ss_total

# Add eta² as a new column
anova_table$eta_squared <- round(eta_squared, 4)
# Filter rows with eta² > 0.10
anova_table_filtered <- anova_table[anova_table$eta_squared > 0.10, ]
# Show the updated ANOVA table
anova_table_filtered
```
